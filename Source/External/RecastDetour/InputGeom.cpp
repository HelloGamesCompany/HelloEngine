...
bool InputGeom::SetChunkyMesh()
{
	...
	if (!m_chunkyMesh)
	{
		CSL::S_Log("buildTiledNavigation: Out of memory 'm_chunkyMesh'.", LogType::ERR);
		return false;
	}
	if (!rcCreateChunkyTriMesh((float*)&m_mesh->vertices[0], (int*)&m_mesh->indices[0], m_mesh->indices.size() / 3, 256, m_chunkyMesh))
	{
		CSL::S_Log("buildTiledNavigation: Failed to build chunky mesh.", LogType::ERR);
		return false;
	}
...
void InputGeom::MergeToMesh(ResourceMesh* new_mesh, float4x4 new_mesh_transform)
{
	...
	MeshInfo meshInfo = new_mesh->GetMeshInfo();
	int new_mesh_vertices_size = meshInfo.vertices.size();
	int new_mesh_indices_size = meshInfo.indices.size();
	int indices_offset = m_mesh->vertices.size();

	// Vertex Merging
	for (size_t i = 0; i < new_mesh_vertices_size; i++)
	{
		float3 vertex = new_mesh_transform.MulPos(meshInfo.vertices[i].position);
		m_mesh->vertices.push_back(vertex);
	}

	//Indices Merging
	for (size_t i = 0; i < new_mesh_indices_size; i++)
		m_mesh->indices.push_back(meshInfo.indices[i] + indices_offset);
}

void InputGeom::SetUpMesh()
{
	m_mesh = new SimpleMesh();
}

void InputGeom::SetMesh(SimpleMesh* mesh)
{
	if (m_mesh)
		RELEASE(m_mesh);
